...
print(output.detach().numpy(), output.shape, "torch模型预测结果")         # (2,7,20)
print(hidden.detach().numpy(), hidden.shape, "torch模型预测隐含层结果")   # (1,2,20)
print("--------------------------------------------")
diy_model = DiyModel(w_ih, w_hh, hidden_size)
x = x[0]
print(x.shape)  # (7,20)
output, hidden = diy_model.forward(x)
print(output, output.shape, "diy模型预测结果")        # (7,20)
print(hidden, hidden.shape, "diy模型预测隐含层结果")  # (1,20)
