...
def load_sentence(path):
    ...
    with open(path, encoding="utf8") as f:
        for line in f:
            sentence = line.strip() #删除开头和结尾处的空格
            sentences.add(" ".join(jieba.cut(sentence)))
    print("获取句子数量：", len(sentences))
    return sentences

# def load_sentenc(path):
#     sentence= set()
#     with open(path,encoding= 'utf-8') as f:
#         for line in f:
#             sentence = line.strip()
#             sentences.add(" ".join(jieba.cut(sentence)))
#         print("huoqujuzishuliang ",len(sentences))
#     return sentences
...
#     kmeans.fit(vectors)
    ...
    # sentence_label_dict = defaultdict(list)   带相同标签的放一起
    ...
    for vector_index, label in enumerate(kmeans.labels_):
        # print("%f------%f",%(vector_index,label))
        vector = vectors[vector_index]   #某句话的向量  这个分明对应了一个标签么
    ...
    #     distance = consine_distance(vector,center)
    ...
    # for label,distance_avg in density_order:
    #     print("33333%s,44444 %f:" %(label,distance_avg))
